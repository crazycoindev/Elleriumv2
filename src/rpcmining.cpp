...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or ellerium.conf setting gen)\n"
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Ellerium is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Ellerium is downloading blocks...");
...
// Release the wallet and main lock while waiting
    ...
    if (pblock->payee != CScript()) {
        ...
        //masternodeObj.push_back(Pair("amount", (int64_t)pblock->vtx[0].vout[1].nValue));
    } else {
