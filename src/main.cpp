...
#if defined(NDEBUG)
#error "Ellerium cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
        nSubsidy = 60001 * COIN;
    } else if (nHeight < 2222 && nHeight > 1) {
        nSubsidy = 3 * COIN;
    } else if (nHeight < Params().LAST_POW_BLOCK() && nHeight > 2222) {
        nSubsidy = 3 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight <= 2222 && nHeight >= 1) {
        ret = blockValue / 30;
    } else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight >= 1000) {
...
{
    RenameThread("ellerium-scriptch");
