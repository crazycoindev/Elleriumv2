...
Ellerium now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<ellerium_address\> \<monthly_payment_ellerium\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ Ellerium, _12_ times over the course of a year totaling _24000_ Ellerium.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<ellerium_address\> \<monthly_payment_ellerium\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` Ellerium.
 ...
 * vote-many          - Vote on a Ellerium initiative
 * vote-alias         - Vote on a Ellerium initiative
 * vote               - Vote on a Ellerium initiative/budget
